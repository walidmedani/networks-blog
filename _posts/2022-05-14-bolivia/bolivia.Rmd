---
title: "2019 Bolivian Political Crisis"
description: |
  A Network Analysis.
author:
  - name: Walid Medani
...
#V(mt)$size <- pr$vector*100  #set node size by PageRank scores.
...
visIgraph(fourcore,idToLabel = TRUE,layout = "layout_nicely") %>%
  visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) 
```
...
visIgraph(onecore,idToLabel = TRUE,layout = "layout_nicely") %>%
  visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) 
...
visIgraph(twocorewt,idToLabel = TRUE,layout = "layout_nicely") %>%
  visOptions(highlightNearest = TRUE, nodesIdSelection = TRUE) 
```

```{r}
#library(blockmodeling)
#mtmat<-as.matrix(as_adj(mt,attr = "nominations"))
...
By conditioning the transitivity (clustering) of the network on size, density, and dyads. We find that in a 1000 random networks, only the variable number of nodes is able to produce a greater value than our observation.
